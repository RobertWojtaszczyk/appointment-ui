...
@Component({
  selector: 'app-time-slot-edit-slot',
  templateUrl: './time-slot-edit-slot.component.html',
  styleUrls: ['./time-slot-edit-slot.component.sass']
})
export class TimeSlotEditSlotComponent implements OnInit {
  @ViewChild(NgForm, {static: false}) productForm: NgForm;

  pageTitle = 'Edit Time Slot Info';
  errorMessage: string;
  timeSlot: TimeSlot;
  ...
  ngOnInit(): void {
    ...
    this.route.parent.data.subscribe( data => {
      if (this.productForm) {
        this.productForm.reset(); // resetowanie stanu formularza(walidacji)
      }
